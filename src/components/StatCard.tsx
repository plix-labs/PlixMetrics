...
export const StatCard: React.FC<StatCardProps> = ({ title, data, type, valueLabel, enableTelegramShare, days, onUserClick }) => {
    ...
    const handleTelegramShare = () => {
        ...
        // Use a space as the URL so it doesn't appear in the message
                    ...
                    if (type === 'user') {
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className={`text-xs sm:text-sm font-bold truncate ${onUserClick ? 'text-white' : 'text-white'}`}>{userItem.user}</p>
                                    <p className="text-[10px] sm:text-xs text-indigo-400 font-medium">{userItem.server_name}</p>
                                </div>
                                <span className="text-sm sm:text-lg font-black text-cyan-400 tabular-nums">
                            ...
                            <div key={libraryItem.rank} className="flex items-center gap-3 py-2 px-3 hover:bg-slate-700/20 rounded-lg mb-1 transition-colors">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    {libraryItem.server_name && (
                                        <p className="text-[10px] sm:text-xs text-slate-500 font-medium">{libraryItem.server_name}</p>
                                    )}
                                </div>
                                <span className="text-sm sm:text-lg font-black text-cyan-400 tabular-nums">
                    ...
                    if (type === 'concurrent') {
                        ...
                        return (
                            <div key={index} className="flex items-center gap-3 py-2 px-3 hover:bg-slate-700/20 rounded-lg mb-1 transition-colors">
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="text-xs sm:text-sm text-white font-bold truncate">{concurrentItem.server_name || 'Unknown Server'}</p>
                                </div>
